---
title: "Two logistics sources disagreed about the first delivery attempt"
...
outcome: "Delivery lead time could be stated in hours and defended, because the timestamp behind it had been checked rather than assumed. The shipments where one source carried nothing came out as one list. The shipments where the two sources disagreed came out as another. Two faults that could then be worked separately."
...
metrics:
  - chart: "gate-funnel"
    ...
    stages:
      ...
      - name: "Compute lead time in hours"
        note: "From the shipment being recorded as received to the first attempt to hand the parcel over."
      - name: "Benchmark delivery lead time"
  ...
  - chart: "coverage-gap"
    label: "The fields the joined view carried, and the one that came apart"
    items:
    ...
    caption: "Shipment number is what the two sources were matched on, and it did that job. Received timestamp, calendar date and merchant details came along as the context a lead time is read in. The comparison itself was on the first delivery attempt, and that is the field the two sources came apart on."
  - chart: "signal-matrix"
...
good as the worse of the two. The received timestamp was not the problem. The
first delivery attempt was, because two logistics sources both carried it and
they did not always say the same thing.

So I went to the shipments rather than to the report. Joining the two datasets
on shipment number turns a lead-time figure nobody can defend into a list you
can count. Here are the shipments where one source has no first-attempt
timestamp at all. Here are the ones where both have it and the values differ.
...
shipment-document event had been deprecated and was no longer publishing, which
matters more than it sounds. Read without knowing that, absent data looks
exactly like nothing having happened. A returns calculation had a quieter
version of the problem. It worked back to the earliest parcel handover by
subtracting a lead time from the date the return closed. That subtraction only
behaves once the lead time is cast to a whole number, so I cast it.
...
then be read next to the risk decision taken on its order. This was logistics
and fulfilment analytics work, alongside measures like on-time delivery and
carrier performance. The risk and abuse work came after it.
